...
; get prefix to shapefiles
origPrefix = 'nhdPlus_SHPs_class'
newPrefix  = 'nhdPlus_SHPs_aggregate'
...
; define the NetCDF file
nc_filename = nhd_root + 'grid2poly/spatialweights_NLDAS12km_NHDPlus2_mod.nc'
...
spawn, 'ls -1d ' + nhd_root + origPrefix + '*', listFiles
nAggregate = n_elements(listFiles)

; loop through aggregations
for iAggregate=0,nAggregate-1 do begin

 ; get the basename
 basename = file_basename(listFiles[iAggregate])
 ...
 nc_filename = nhd_root + 'conusPfafstetter_aggregate' + cId + '.nc'
 print, 'Reading ' + nc_filename
 ...
 varid = ncdf_varid(ncid, 'pfafClass_hru')
 ...
 for iFile=0,n_elements(nhdFiles)/2-1 do begin
  ...
  for iType=0,n_elements(shpType)-1 do begin
   ...
   shpFile_orig = nhd_root + origPrefix + cId + '/' + shpType[iType] + nhdFiles[0,iFile] + '_' + nhdFiles[1,iFile] + '.shp'
   shpFile_new  = nhd_root + newPrefix + cId + '/' + shpType[iType] + nhdFiles[0,iFile] + '_' + nhdFiles[1,iFile] + '.shp'
   print, shpFile_orig
   ...
   ; define the aggregated shapefiles
   if(shpType[iType] eq 'Catchment_')then hruShpFile_agg = shpFile_new 
   if(shpType[iType] eq 'Flowline_')then  segShpFile_agg = shpFile_new 
