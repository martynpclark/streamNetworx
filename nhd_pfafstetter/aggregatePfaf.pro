...
; Define shapefile
root_filepath = '/Volumes/d1/mclark/HUC/conus/'
shp_huc12     = root_filepath + 'huc12.shp'
...
ncid = ncdf_open(nc_filename, /nowrite)
 ...
 ncdf_varget, ncid, ncdf_varid(ncid, 'hruId'), comId_cat
...
ixNegative = where(strmid(strtrim(pfafCode_cat,2),0,1) eq '-', nNegative)
if(nNegative gt 0)then begin
 print, nNegative
 print, pfafCode_cat[ixNegative]
...
pfafVec_hru[sort(hruId_mizu)] = strtrim(pfafCode_cat[sort(comId_cat)], 2)
...
if(file_test(saveFile) eq 0)then begin
 ...
 ; define the NetCDF file
 nc_filepath  = '/Volumes/d1/mclark/NHD_Plus/ancillary_data/'
 nc_filetemp = nc_filepath + 'conusPfafstetter_coastalCatchments.nc'
 ...
 if(file_test(nc_filename) eq 0)then begin
  ...
  spawn, 'cp -f ' + nc_filetemp + ' ' + nc_filename
  ncid = ncdf_open(nc_filename, /write)
  ncdf_control, ncid, /redef
 
   ; hru classification
   dimId = ncdf_dimid(ncid, 'hru')
   varid = ncdf_vardef(ncid, 'pfafClass_cat', [dimid], /string)
 ...
 varid = ncdf_varid(ncid, 'pfafClass_cat')         ; note: Pfaf classification sorted to match hruId_mizu
